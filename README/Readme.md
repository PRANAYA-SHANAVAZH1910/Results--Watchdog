...
What it does
Launches Chromium and logs into https://www.pesuacademy.com/Academy/
...
If configured with the scheduler, repeats this automatically every few hours overnight, and stops on its own the moment results are found — no manual babysitting required
Project structure
Agentic_workflow/
...
Make sure the macro is enabled, MacroDroid has SMS permission, and battery optimization is disabled for the app (Settings → Apps → MacroDroid → Battery → Unrestricted) so it keeps running in the background overnight
...
The scheduler runs the main script repeatedly within a defined time window (e.g. every 2 hours from 4:30 PM today to 10:00 AM tomorrow), and stops early the moment results are detected as published.
...
INTERVAL_HOURS / start / end	scheduler.py	How often and how long the scheduler runs overnight
RESULTS_READY_EXIT_CODE	both files (must match)	Internal signal used to tell the scheduler to stop early
Troubleshooting
ModuleNotFoundError — your virtual environment isn't activated, or the package isn't installed inside it. Run source .venv/bin/activate then pip3 install <package>.
